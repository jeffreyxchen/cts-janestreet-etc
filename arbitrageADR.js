...
function doNOKUSArbitrage (nokus, nokfh) {
  ...
  if (valNOKUS < nokfh) {
    client.write(JSON.stringify({"type": "add", "order_id": new Date(), "symbol": "NOKUS", "dir": "BUY", "price": nokfh, "size": 5})+"\n");
    client.write(JSON.stringify({"type": "convert", "order_id": new Date(), "symbol": "NOKUS", "dir": "SELL", "size": 5})+"\n");
    client.write(JSON.stringify({"type": "add", "order_id": new Date(), "symbol": "NOKFH", "dir": "SELL", "price": nokus, "size": 5})+"\n");
  }
  else {
    var tempAMT = amt;
